...
<section>
    <?php foreach($contents as $content) { if(get_class($content) == 'article') { ?>
        <article class="front-article" id='<?php echo "Vsebina_".$content->id; ?>'>
            <header>
                <h3><a href="<?php echo base_url()."vsebina/".$content->id."/urejanje"; ?>" data-hover="<?php echo $content->name; ?>"><?php echo $content->name; ?></a></h3>
            ...
            <section>
                ...
                <ul class="icon tags-icon" style="opacity: 0.6;">
                    <?php foreach($content->tags as $tag) echo "<li style='display:inline-block; margin-right:10px;'>".$tag."</li>";?>
                </ul>
            </section>

            <footer>
                <nav class="cl-effect-16" id="cl-effect-16">
                    <a class="icon magnify-icon" href='<?php echo base_url()."vsebina/".$content->id."/".$content->slug; ?>' data-hover="pogled">pogled</a>
                    <?php if($content->owner) { ?>
                        <a class="icon edit-icon" href='<?php echo base_url()."vsebina/".$content->id."/urejanje/"; ?>' data-hover="urejanje">urejanje</a>
